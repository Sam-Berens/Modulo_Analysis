...
% GETTPATTERNS_ROIRES  Extract Alpha00 T-image patterns at ROI resolution.
%
%   [Data, pCover] = GETTPATTERNS_ROIRES(G, subjectId, roiId)
%
%   Loads a fixed set of first-level SPM T-statistic images for a subject
%   (six integers) and samples them into the voxel grid of a native-space
%   ROI mask ("ROI resolution"). The output is a matrix of T-values with
%   one row per ROI voxel and one column per T-image/condition.
%
